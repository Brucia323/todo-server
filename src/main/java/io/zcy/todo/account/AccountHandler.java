...
public class AccountHandler {
        ...
        .flatMap(
              ...
              return service
                  ...
                  .flatMap(
                        ...
                        boolean checkpw =
                            BCrypt.checkpw(accountDTO.getPassword(), account.getPasswordHash());
                        if (!checkpw) {
                          return ServerResponse.status(HttpStatus.UNAUTHORIZED).build();
                        }
                        String token =
  ...
  public Mono<ServerResponse> createUser(ServerRequest request) {
    log.info("{}", request);
    Mono<AccountDTO> accountDTO =
        request.bodyToMono(AccountDTO.class).flatMap(service::createUser).map(AccountDTO::new);
  ...
  public Mono<ServerResponse> updateUser(ServerRequest request) {
    log.info("{}", request);
    Mono<AccountDTO> accountDTO =
        request.bodyToMono(AccountDTO.class).flatMap(service::updateUser).map(AccountDTO::new);
