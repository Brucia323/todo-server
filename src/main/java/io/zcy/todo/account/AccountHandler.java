...
public class AccountHandler {
  ...
  public Mono<ServerResponse> checkPassword(ServerRequest request) {
    ...
    return password
        .map(s -> service.checkPassword(id, s).flatMap(
            result -> result ? ServerResponse.ok().build() : ServerResponse.status(HttpStatus.UNAUTHORIZED).build()))
        .orElseGet(() -> ServerResponse.status(HttpStatus.UNAUTHORIZED).build());
  }
}
